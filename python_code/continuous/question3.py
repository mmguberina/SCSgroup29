...
# we need to do these collisions more efficiently
    actActNeig = {i:[] for i in range(nOfActiveParticles)}
    actPasNeig = {i:[] for i in range(nOfActiveParticles)}
    pasActNeig = {i:[] for i in range(nOfPassiveParticles)}
    pasPasNeig = {i:[] for i in range(nOfPassiveParticles)}
    ...
    for step in range(n):
        ...
        for i in range(nOfActiveParticles):
            ...
            actActNeig[i] = [part for part in range(nOfActiveParticles) if rnorms[part] < 4 * particle_radius]
            rnormsPas = np.linalg.norm(rPas, axis=1).reshape((nOfPassiveParticles,1))
            actPasNeig[i] = [part for part in range(nOfPassiveParticles) if rnormsPas[part] < 4 * particle_radius]
            # we need rhat
            r_hat  = r / rnorms
            r_hat[i] = np.zeros(2)
...
nOfActiveParticles = 100
nOfPassiveParticles = 1100
#rot_dif_T = 0.2
#trans_dif_T = 0.2
#v = 1
#ni = 0.0002
nis = [1.4*np.pi, 0.5*np.pi, 0.03*np.pi]
ni = nis[0]
v = 0.05
# Total time.
T = 50
...
for timestep in range(N):
   # for i in range(nOfParticles):
    if timestep % 30 == 0 and timestep > 201:
...
#        ax.scatter(x[:, timestep], y[:, timestep], s=s, color='red')
        ...
        #for sh in range(1, 64,2):
        xsfade = x[:, timestep - 200:timestep]
        xsfade = xsfade.reshape((xsfade.shape[0] * xsfade.shape[1],))
        ysfade = y[:, timestep - 200:timestep]
        ysfade = ysfade.reshape((ysfade.shape[0] * ysfade.shape[1],))

        ax.scatter(xsfade, ysfade, color='blue', alpha=0.15)
...
animation.save('question3BIGNNICElonger_=' + str(ni) + str(nOfPassiveParticles) +'.mp4')
plt.show()
